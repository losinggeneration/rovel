...
func TestMapKeyShiftTab(t *testing.T) {
	key, ok := mapKey(sdl.K_TAB, sdl.KMOD_LSHIFT)
	if !ok {
...
func TestMapMod(t *testing.T) {
	tests := []struct {
		...
		{0, 0},
		{sdl.KMOD_LSHIFT, tevent.ModShift},
		{sdl.KMOD_RSHIFT, tevent.ModShift},
		{sdl.KMOD_LALT, tevent.ModAlt},
		{sdl.KMOD_RALT, tevent.ModAlt},
		{sdl.KMOD_LCTRL, tevent.ModCtrl},
		{sdl.KMOD_RCTRL, tevent.ModCtrl},
		{sdl.KMOD_LSHIFT | sdl.KMOD_LALT, tevent.ModShift | tevent.ModAlt},
		{sdl.KMOD_LSHIFT | sdl.KMOD_LCTRL, tevent.ModShift | tevent.ModCtrl},
		{sdl.KMOD_LALT | sdl.KMOD_LCTRL, tevent.ModAlt | tevent.ModCtrl},
		{sdl.KMOD_LSHIFT | sdl.KMOD_LALT | sdl.KMOD_LCTRL, tevent.ModShift | tevent.ModAlt | tevent.ModCtrl},
	}
...
func TestMapMouseButton(t *testing.T) {
	tests := []struct {
		...
		{1, tevent.MouseButtonLeft},
		{2, tevent.MouseButtonMiddle},
		{3, tevent.MouseButtonRight},
		{4, tevent.MouseButtonNone},
		{5, tevent.MouseButtonNone},
		{0, tevent.MouseButtonNone},
		{99, tevent.MouseButtonNone},
	}
...
func TestWheelEventSteps(t *testing.T) {
	tests := []struct {
		...
		{"up", sdl.MouseWheelEvent{Y: 3}, 3, tevent.MouseButtonWheelUp},
		{"down", sdl.MouseWheelEvent{Y: -2}, 2, tevent.MouseButtonWheelDown},
		{"down one", sdl.MouseWheelEvent{Y: -1}, 1, tevent.MouseButtonWheelDown},
		{"up one", sdl.MouseWheelEvent{Y: 1}, 1, tevent.MouseButtonWheelUp},
		{"horizontal up", sdl.MouseWheelEvent{X: 2}, 2, tevent.MouseButtonWheelUp},
		{"horizontal down", sdl.MouseWheelEvent{X: -1}, 1, tevent.MouseButtonWheelDown},
		{"zero", sdl.MouseWheelEvent{}, 0, tevent.MouseButtonNone},
		{"flipped up", sdl.MouseWheelEvent{Y: -1, Direction: sdl.MOUSEWHEEL_FLIPPED}, 1, tevent.MouseButtonWheelUp},
		{"flipped down", sdl.MouseWheelEvent{Y: 1, Direction: sdl.MOUSEWHEEL_FLIPPED}, 1, tevent.MouseButtonWheelDown},
	}
...
func TestHandleCtrlModifiedKeyLetters(t *testing.T) {
	b := &Backend{Core: NewCore(DefaultOptions())}

	for _, k := range []sdl.Keycode{sdl.K_A, sdl.K_C, sdl.K_Z} {
		if !b.handleCtrlModifiedKey(k, sdl.KMOD_LCTRL) {
			t.Errorf("handleCtrlModifiedKey(%v, ctrl) = false, want true", k)

			continue
		}

		ev := b.ReadEvent()
		got, ok := ev.(tevent.KeyEvent)
		if !ok {
			t.Fatalf("ReadEvent = %T, want tevent.KeyEvent", ev)
		}

		if got.Key != tevent.KeyRune || got.Mod != tevent.ModCtrl {
			t.Errorf("ctrl+%v emitted %+v, want KeyRune + ModCtrl", k, got)
		}
	}
}
